...
扩展清单按项目权限决策声明全站访问，但当前版本没有常驻内容脚本，也不会监控或记录浏览历史。只有用户通过工具栏、右键菜单或快捷键明确触发收藏动作，或在全页管理器中主动刷新选中书签元数据时，扩展才读取对应页面的 URL、标题、描述和封面元数据。OpenBookmark 不包含账号、遥测、分析 SDK 或网络后端。
